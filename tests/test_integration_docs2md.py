...
class TestIntegrationDocs2md(unittest.TestCase):
    ...
    def test_skipdir_tag(self):
        """Test directory with doc2md#skipdir tag is skipped"""
        # Create directory with skipdir tag
        skip_dir = os.path.join(self.test_data_dir, 'skip_dir')
        os.makedirs(skip_dir)
        
        # Create README with skipdir tag
        self.create_readme(skip_dir, '# Skip This\ndoc2md#skipdir')
    ...
    def test_skipdir_tag_subdirectories(self):
        """Test skipdir tag also skips subdirectories"""
        # Create parent with skipdir
        parent_dir = os.path.join(self.test_data_dir, 'parent')
        child_dir = os.path.join(parent_dir, 'child')
        os.makedirs(child_dir)
        
        # Create README with skipdir in parent
        self.create_readme(parent_dir, '# Parent\ndoc2md#skipdir')
        self.create_readme(child_dir, '# Child\ntest.html')
        
        # Create test files
        self.create_test_file(child_dir, 'test.html', '<html>Test</html>')
        ...
        # Verify child was not processed
        md_file = os.path.join(child_dir, 'test.md')
        self.assertFalse(os.path.exists(md_file))
    ...
    def test_file_referenced_in_readme(self):
        ...
        # Create README referencing file
    ...
    def test_file_not_referenced_no_mask(self):
        ...
        # Create README without referencing file
        self.create_readme(test_dir, '# Documentation\nNo files mentioned')
    ...
    def test_skipfile_tag(self):
        ...
        # Create README with skipfile tag
        self.create_readme(
            test_dir,
    ...
    def test_name_conflict_resolution(self):
        ...
        self.assertTrue(html_target.endswith('_html.md') or html_target.endswith('test.md'))
        self.assertTrue(xml_target.endswith('_xml.md') or xml_target.endswith('test.md'))
